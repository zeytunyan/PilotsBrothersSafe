...
1. Для четных полей можно изменить положение определенных ручек, просто повернув все ручки в их столбце и строке. Тем самым легко для любой ручки получить желаемое положение и выиграть.
2. 



## Дополнительная информация о реализации
...
Данная реализация устроена так, что, вероятно, может работать не только для полей с размером N до 10, но и для вообще любого размера, а также при соотношении сторон поля не только NxN, но MxN. Однако интерфейс не позволяет задать такие параметры поля, поскольку тогда его внешний вид оставляет желать лучшего.
